...
template <typename DType>
void NonLinearTransformAndNormalization_xywh(
...
template <typename DType>
void NonLinearTransformAndNormalization_xyxy(
  const Tensor<cpu, 2, DType> &ex_rois,
  const Tensor<cpu, 2, DType> &gt_rois,
  Tensor<cpu, 2, DType> &targets,
  const Tensor<cpu, 1, DType> &bbox_mean,
...
struct ProposalTargetParam : public dmlc::Parameter<ProposalTargetParam> {
  ...
  DMLC_DECLARE_PARAMETER(ProposalTargetParam) {
    ...
    DMLC_DECLARE_FIELD(xywh).set_default(true).describe("bbox decode type");
