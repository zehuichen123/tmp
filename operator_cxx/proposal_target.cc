...
inline void SampleROI(const Tensor<cpu, 2, DType> &all_rois,
  ...
  */
  ...
  if(xywh){
    NonLinearTransformAndNormalization_xywh(rois_tmp, gt_bboxes_tmp, targets, bbox_mean, bbox_std);
  }
  else{
    NonLinearTransformAndNormalization_xyxy(rois_tmp, gt_bboxes_tmp, targets, bbox_mean, bbox_std);
  }
...
template <typename DType>
void NonLinearTransformAndNormalization_xywh(const Tensor<cpu, 2, DType> &ex_rois,
...
template <typename DType>
void NonLinearTransformAndNormalization_xyxy(const Tensor<cpu, 2, DType> &ex_rois,
                                             const Tensor<cpu, 2, DType> &gt_rois,
                                             Tensor<cpu, 2, DType> &targets,
                                             const Tensor<cpu, 1, DType> &bbox_mean,
                                             const Tensor<cpu, 1, DType> &bbox_std) {
  index_t num_roi = ex_rois.size(0);
  for (index_t i = 0; i < num_roi; ++i) {
      DType ex_x1 = ex_rois[i][0];
      DType ex_y1 = ex_rois[i][1];
      DType ex_x2 = ex_rois[i][2];
      DType ex_y2 = ex_rois[i][3];

      DType gt_x1 = gt_rois[i][0];
      DType gt_y1 = gt_rois[i][1];
      DType gt_x2 = gt_rois[i][2];
      DType gt_y2 = gt_rois[i][3];

      DType ex_width  = ex_x2 - ex_x1 + 1.f;
      DType ex_height = ex_y2 - ex_y1 + 1.f;
      targets[i][0]   = (gt_x1 - ex_x1) / (ex_width + 1e-14f);
      targets[i][1]   = (gt_y1 - ex_y1) / (ex_height + 1e-14f);
      targets[i][2]   = (gt_x2 - ex_x2) / (ex_width + 1e-14f);
      targets[i][3]   = (gt_y2 - ex_y2) / (ex_height + 1e-14f);
      targets[i] -= bbox_mean;
      targets[i] /= bbox_std;
  }
}
